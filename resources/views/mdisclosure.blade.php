@include('includes.header')

<div class="edu-breadcrumb-area">
    <div class="container">
        <div class="breadcrumb-inner">
            <div class="page-title">
                <h1 class="title">Mandatory Public Disclosure</h1>
            </div>
            <ul class="edu-breadcrumb">
                <li class="breadcrumb-item"><a href="/">Home</a></li>
                <li class="separator"><i class="icon-angle-right"></i></li>
                <li class="breadcrumb-item active" aria-current="page">Mandatory Public Disclosure</li>
            </ul>
        </div>
    </div>
    <ul class="shape-group">
        <li class="shape-1">
            <span></span>
        </li>
        <li class="shape-2 scene" style="transform: translate3d(0px, 0px, 0px) rotate(0.0001deg); transform-style: preserve-3d; backface-visibility: hidden; pointer-events: none;"><img data-depth="2" src="assets/images/about/shape-13.png" alt="shape" style="transform: translate3d(8.6px, -30.2px, 0px); transform-style: preserve-3d; backface-visibility: hidden; position: relative; display: block; left: 0px; top: 0px;"></li>
        <li class="shape-3 scene" style="transform: translate3d(0px, 0px, 0px) rotate(0.0001deg); transform-style: preserve-3d; backface-visibility: hidden; pointer-events: none;"><img data-depth="-2" src="assets/images/about/shape-15.png" alt="shape" style="transform: translate3d(-4.7px, 6.3px, 0px); transform-style: preserve-3d; backface-visibility: hidden; position: relative; display: block; left: 0px; top: 0px;"></li>
        <li class="shape-4">
            <span></span>
        </li>
        <li class="shape-5 scene" style="transform: translate3d(0px, 0px, 0px) rotate(0.0001deg); transform-style: preserve-3d; backface-visibility: hidden; pointer-events: none;"><img data-depth="2" src="assets/images/about/shape-07.png" alt="shape" style="transform: translate3d(5.7px, -31px, 0px); transform-style: preserve-3d; backface-visibility: hidden; position: relative; display: block; left: 0px; top: 0px;"></li>
    </ul>
</div>


<div class="container content-sm mt-5">
    <div class="row margin-bottom-30">
        <div class="col-md-12 md-margin-bottom-40">
              <h3 class="title text-capitalize font-weight-light text-dark text-center mb-5">Mandatory Public 
            <span class="font-weight-bold">Disclosure</span>
        </h3>
            <div class="row">
                <div class="col-sm-12">
                    <div class="table-responsive margin-bottom-30">
                        <p class="font-weight-bold">A. GENERAL INFORMATION</p><br>
                        <table class="table table-primary table-bordered" align="center">
                            <tbody><tr>
                                <th>S.No</th>
                                <th>INFORMATION</th>
                                <th>DETAILS</th>
                            </tr>
                            <tr>
                                <td>01.</td>
                                <td>NAME OF THE SCHOOL</td>
                                <td>GOLDEN HARRIER SCHOOL</td>
                            </tr>
                             <tr>
                                <td>02.</td>
                                <td>AFFILIATION NO</td>
                                <td>531446</td>
                            </tr>
                            <tr>
                                <td>03.</td>
                                <td>SCHOOL CODE</td>
                                <td>41418</td>
                            </tr>
                            <tr>
                                <td>04.</td>
                                <td>COMPLETE ADDRESS</td>
                                <td>JHUNDPUR ROAD,JAKHOLI MOR,SONEPAT-131023</td>
                            </tr>
                            <tr>
                                <td>05.</td>
                                <td>PRINCIPAL NAME &amp; QUALIFICATION</td>
                                <td>DR. ANKUR ROHILLA<span class="font-weight-bold">&nbsp;(M.PHARMA,M.SC,Ph.D)</span></td>
                            </tr>
                             <tr>
                                <td>06.</td>
                                <td>SCHOOL EMAIL ID</td>
                                <td>goldenharriers@gmail.com</td>
                            </tr>
                            <tr>
                                <td>07.</td>
                                <td>CONTACT DETAILS</td>
                                <td>9896064802,9896064902</td>
                            </tr>
                        </tbody></table>
                  <p class="font-weight-bold">B. DOCUMENTS AND INFORMATION</p><br>
                  <table class="table table-primary table-bordered" align="center">
                            <tbody><tr>
                                <th>S.No</th>
                                <th>DOCUMENT/INFORMATION</th>
                                <th>DOCUMENTS</th>
                            </tr>
                            <tr>
                                <td>01.</td>
                                <td>AFFILIATION LETTER</td>
                                <td><a data-abc="true">View</a></td>
                            </tr>
                             <tr>
                                <td>02.</td>
                                <td>SOCIETIES/TRUST CERTIFICATE/COMPANY</td>
                                <td><a data-abc="true">View</a></td>
                            </tr>
                            <tr>
                                <td>03.</td>
                                <td>NO OBJECTION CERTIFICATE(NOC)</td>
                                <td><a data-abc="true">View</a></td>
                            </tr>
                            <tr>
                                <td>04.</td>
                                <td>RECOGNITION CERTIFIATE UNDER RTE ACT 2009</td>
                                <td><a data-abc="true">View</a></td>
                            </tr>
                            <tr>
                                <td>05.</td>
                                <td>BUILDING SAFETY CERTIFICATE</td>
                                <td><a data-abc="true">View</a></td>
                            </tr>
                             <tr>
                                <td>06.</td>
                                <td>FIRE SAFETY CERTIFICATE</td>
                                <td><a data-abc="true">View</a></td>
                            </tr>
                            <tr>
                                <td>07.</td>
                                <td>DEO CERTIFICATE SUBMITTED BY THE SCHOOL FOR AFFILIATION</td>
                               <td><a data-abc="true">View</a></td>
                            </tr>
                       <tr>
                                <td>08.</td>
                                <td>WATER,HEALTH AND SANITATION CERTIFICATES</td>
                               <td><a data-abc="true">View</a></td>
                            </tr>
                        </tbody></table>
                          <p class="font-weight-bold">C. RESULT AND ACADEMICS</p><br>
                  <table class="table table-primary table-bordered" align="center">
                            <tbody><tr>
                                <th>S.No</th>
                                <th>DOCUMENT/INFORMATION</th>
                                <th>DOCUMENTS</th>
                            </tr>
                            <tr>
                                <td>01.</td>
                                <td>FEE STRUCTURE OF THE SCHOOL</td>
                                <td><a href="fee.aspx" data-abc="true">View</a></td>
                            </tr>
                             <tr>
                                <td>02.</td>
                                <td>ANNUAL ACADEMIC CALENDAR</td>
                                <td><a href="Calendar.aspx" data-abc="true">View</a></td>
                            </tr>
                            <tr>
                                <td>03.</td>
                                <td>LIST OF SCHOOL MANAGEMENT COMMITTEE (SMC)</td>
                                <td><a href="smc.aspx" data-abc="true">View</a></td>
                            </tr>
                            <tr>
                                <td>04.</td>
                                <td>LIST OF PARENTS TEACHERS ASSOCIATION(PTA) MEMBERS</td>
                                <td><a href="smc.aspx" data-abc="true">View</a></td>
                            </tr>
                            <tr>
                                <td>05.</td>
                                <td>LAST THREE-YEAR RESULT OF THE BOARD EXAMINATION</td>
                                <td><a data-abc="true">See Below</a></td>
                            </tr>
                             
                        </tbody></table>
                         <p class="font-weight-bold">RESULT CLASS-X</p><br>
                  <table class="table table-primary table-bordered" align="center">
                            <tbody><tr>
                                <th>S.No</th>
                                <th>YEAR</th>
                                <th>NO. OF REGISTERED STUDENTS</th>
                                <th>NO. OF STUDENTS PASSED</th>
                                <th>PASS PERCENTAGE</th>
                                <th>REMARKS</th>
                            </tr>
                            <tr>
                                <td>01.</td>
                                <td>2018-19</td>
                                <td>63</td>
                                <td>63</td>
                                <td>100</td>
                                <td></td>
                            </tr>
                        <tr>
                                <td>02.</td>
                                <td>2019-20</td>
                                <td>61</td>
                                <td>61</td>
                                <td>100</td>
                                <td></td>
                            </tr>
                      <tr>
                                <td>03.</td>
                                <td>2020-21</td>
                                <td>91</td>
                                <td></td>
                                <td></td>
                                <td></td>
                            </tr>
                      </tbody></table>
                         <p class="font-weight-bold">RESULT CLASS-XII</p><br>
                  <table class="table table-primary table-bordered" align="center">
                           <tbody><tr>
                                <th>S.No</th>
                                <th>YEAR</th>
                                <th>NO. OF REGISTERED STUDENTS</th>
                                <th>NO. OF STUDENTS PASSED</th>
                                <th>PASS PERCENTAGE</th>
                                <th>REMARKS</th>
                            </tr>
                            <tr>
                                <td>01.</td>
                                <td>2019-20</td>
                                <td>49</td>
                                <td>49</td>
                                <td>100</td>
                                <td></td>
                            </tr>
                        <tr>
                                <td>02.</td>
                                <td>2020-2021</td>
                                <td>69</td>
                                <td></td>
                                <td></td>
                                <td></td>
                            </tr>  
                      </tbody></table>
                          <p class="font-weight-bold">D. STAFF TEACHING</p><br>
                  <table class="table table-primary table-bordered" align="center">
                            <tbody><tr>
                                <th>S.No</th>
                                <th>INFORMATION</th>
                                <th>DETAILS</th>
                            </tr>
                            <tr>
                                <td>01.</td>
                                <td>PRINCIPAL</td>
                                <td><a href="principal-message.aspx" data-abc="true">Click</a></td>
                            </tr>
                             <tr>
                                <td rowspan="4">02.</td>
                                <td>TOTAL NO. OF TEACHERS</td>
                                <td>55</td>
                            </tr>
                       <tr>
                                <td>PGT</td>
                                <td>14</td>
                            </tr>
                      <tr>
                                <td>TGT</td>
                                <td>17</td>
                            </tr>
                      <tr>
                                <td>PRT</td>
                                <td>15</td>
                            </tr>
                            <tr>
                                <td>03.</td>
                                <td>TEACHERS SECTION RATIO</td>
                                <td>1:1.5</td>
                            </tr>
                            <tr>
                                <td>04.</td>
                                <td>DETAILS OF SPECIAL EDUCATOR</td>
                                <td>Mr. Gaurav Dagar (M.A,B.Ed(Psychology))</td>
                            </tr>
                            <tr>
                                <td>05.</td>
                                <td>DETAILS OF COUNSELLOR AND WELLNESS TEACHER</td>
                                <td>Dr. Jyoti Chikkara(MBBS)</td>
                            </tr>
                             
                        </tbody></table>
                         <p class="font-weight-bold">E. SCHOOL INFRASTRUCTURE</p><br>
                        <table class="table table-primary table-bordered" align="center">
                            <tbody><tr>
                                <th>S.No</th>
                                <th>INFORMATION</th>
                                <th>DETAILS</th>
                            </tr>
                            <tr>
                                <td>01.</td>
                                <td>TOTAL CAMPUS AREA OF THE SCHOOL(IN SQUARE MTR)</td>
                                <td>21347</td>
                            </tr>
                             <tr>
                                <td>02.</td>
                                <td>NO. AND SIZE OF THE CLASS ROOMS (IN SQ MTR)</td>
                                <td>39 (40.13 SQ MTR Each)</td>
                            </tr>
                            <tr>
                                <td>03.</td>
                                <td>NO. AND SIZE OF LABORATORIES INCLUDING COMPUTER LABS(IN SQ MTR)</td>
                                <td>7 (56.85 SQ MTR-81.94 SQ MTR Each)</td>
                            </tr>
                            <tr>
                                <td>04.</td>
                                <td>INTERNET FACILITY(Y/N)</td>
                                <td>Y</td>
                            </tr>
                            <tr>
                                <td>05.</td>
                                <td>NO. OF GIRLS TOILETS</td>
                                <td>8</td>
                            </tr>
                             <tr>
                                <td>06.</td>
                                <td>NO. OF BOYS TOILETS</td>
                                <td>8</td>
                            </tr>
                            <tr>
                                <td>07.</td>
                                <td>CBSE SCHOOL INSPECTION VIDEO</td>
                                <td><a href="https://web.archive.org/web/20210917184717/https://youtu.be/P4XRGWkOc14" data-abc="true">View</a></td>
                            </tr>
                        </tbody></table>
                    </div>
             </div>
            </div>
        </div>
</div>
</div>



@include('includes.footer')